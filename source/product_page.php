...
if(isset($_POST['submit'])){
    ...
    if($checkResult){
        $msg = "Producto ya agregado!! Ir al carrito por favor.";
    }
    else {
...
<!--                <input type="hidden" name="productId" value="--><?//= $pId; ?><!--">-->
               <?php if($row['productFor']=="Men's Fashion" || $row['productFor']=="Women's Fashion" || $row['productFor']=="Baby's Fashion"): ?>
                 <label ><b>Talla </b></label><br>
         <select name="productSize" style="border: 1px solid gray;border-radius: 10px;cursor: pointer;width: 20%;height: 30px" required>
                    <option name="size" selected disabled>Seleciona talla</option>
                    <option name="size" value="Pequeño">S</option>
                    <option name="size" value="Mediano">M</option>
                    <option name="size" value="Largo">L</option>
                    <option name="size" value="Extra Largo">XL</option>
                 ...
                 <h4 data-currency-iso="BDT" style="margin-top: 5%"><b>Precio:</b> $ <?= $row['price']; ?></h4>

                 <button name="submit" class="btn" style="background-color: #222222;color: #F79622;font-size: 20px;float: right;">Añadir a la cesta </button><br><br>
            </form>
